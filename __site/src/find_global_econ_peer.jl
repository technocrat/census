include("setup.jl")
world = CSV.read(datadir() * "/world_pop24.csv", DataFrame)
partialsdir(args...) = projectdir("_layout/partials", args...)
sort!(world, :country)
eu = filter(:country => x -> x ∈ EU, world)
sort!(eu, :population, rev=true)


function output_nations_compared_to_eu_table(nations::Vector{Vector{String}}, 
    ...
    for (i, nation) in enumerate(nations)
        ...
        output_file = joinpath(base_path, "$(safe_title)_eu_compare_table.html")
        ...
        try
            ...
            push!(results, "HTML population league table for $(nation_title) written to $(output_file)")
        catch e
